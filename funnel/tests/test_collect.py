...
def test_parse_job_cards_skips_empty_static_text():
    # An empty StaticText must not become the company_name.
    snap = "\n".join(
        [
            'uid=4_0 heading "full details of Backend Engineer" level="3"',
            'uid=4_1 button "full details of Backend Engineer"',
            'uid=4_2 StaticText ""',
            'uid=4_3 StaticText "Acme Corp"',
            'uid=4_4 StaticText "Remote"',
        ]
    )
    cards = parse_job_cards(snap)
    assert len(cards) == 1
